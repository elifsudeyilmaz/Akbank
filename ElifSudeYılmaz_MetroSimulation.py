    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        while pq:
            ...
            if mevcut.idx in ziyaret_edildi:
                if toplam_sureler[mevcut.idx] <= toplam_sure:
                    continue
            ziyaret_edildi.add(mevcut.idx)
            toplam_sureler[mevcut.idx] = toplam_sure

            yeni_yol = yol + [mevcut]

            if mevcut == hedef:
                return (yeni_yol, toplam_sure)

            for komsu, sure in mevcut.komsular:
                tahmini_sure = toplam_sure + sure
                heapq.heappush(pq, (tahmini_sure, id(komsu), komsu, yeni_yol))
        return None
